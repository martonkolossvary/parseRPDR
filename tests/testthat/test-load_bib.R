###############################################################################
## Title: Test load_bib() function
## Project: parseRPDR
## Description: Test load_bib() function
...
d_fname <- list.files(folder_raw, full.names = TRUE)[grep("bib.txt", list.files(folder_raw), ignore.case = TRUE)]
d_s <- suppressMessages(load_bib(d_fname, nThread = 1))
d_p <- suppressMessages(load_bib(d_fname, nThread = 2))


test_that("load_bib run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "bib.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "bib.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "bib.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
