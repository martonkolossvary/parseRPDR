###############################################################################
## Title: Test load_rdt() function
## Project: parseRPDR
## Description: Test load_rdt() function
...
d_s <- suppressMessages(load_rdt(d_fname, nThread = 1))
d_p <- suppressMessages(load_rdt(d_fname, nThread = 2))


test_that("load_rdt run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "rdt.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "rdt.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "rdt.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
