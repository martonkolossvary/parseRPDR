###############################################################################
## Title: Test load_rfv() function
## Project: parseRPDR
## Description: Test load_rfv() function
...
d_s <- suppressMessages(load_rfv(d_fname, nThread = 1))
d_p <- suppressMessages(load_rfv(d_fname, nThread = 2))


test_that("load_rfv run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "rfv.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "rfv.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "rfv.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
