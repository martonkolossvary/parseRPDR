###############################################################################
## Title: Test load_lab() function
## Project: parseRPDR
## Description: Test load_lab() function
...
d_s <- suppressMessages(load_lab(d_fname, nThread = 1))
d_p <- suppressMessages(load_lab(d_fname, nThread = 2))


test_that("load_lab run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "lab.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "lab.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "lab.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
