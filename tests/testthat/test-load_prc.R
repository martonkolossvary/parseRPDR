###############################################################################
## Title: Test load_prc() function
## Project: parseRPDR
## Description: Test load_prc() function
...
## Check legacy table -----
expect_true({
  d_fname <- list.files(paste0(folder_raw, "LEGACY/"), full.names = TRUE)[grep("badprocedures.txt", list.files(paste0(folder_raw, "LEGACY/")), ignore.case = TRUE)]
  d_s <- suppressMessages(load_prc(d_fname, nThread = 1))
  TRUE
})
...
test_that("load_prc run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "prc.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "prc.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "prc.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
