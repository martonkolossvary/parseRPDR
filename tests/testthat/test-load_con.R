###############################################################################
## Title: Test load_con() function
## Project: parseRPDR
## Description: Test load_con() function
...
d_s <- suppressMessages(load_con(d_fname, nThread = 1))
d_p <- suppressMessages(load_con(d_fname, nThread = 2))


test_that("load_con run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "con.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "con.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "con.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
