###############################################################################
## Title: Test load_mcm() function
## Project: parseRPDR
## Description: Test load_mcm() function
...
d_fname <- list.files(folder_raw, full.names = TRUE)[grep("mcm.txt", list.files(folder_raw), ignore.case = TRUE)]
d_s <- suppressMessages(load_mcm(d_fname, nThread = 1))
d_p <- suppressMessages(load_mcm(d_fname, nThread = 2))


test_that("load_mcm run using sequential and parallel loading returns same results", {
