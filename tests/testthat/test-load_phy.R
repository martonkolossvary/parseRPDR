###############################################################################
## Title: Test load_phy() function
## Project: parseRPDR
## Description: Test load_phy() function
...
d_s <- suppressMessages(load_phy(d_fname, nThread = 1))
d_p <- suppressMessages(load_phy(d_fname, nThread = 2))


test_that("load_phy run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "phy.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "phy.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "phy.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
