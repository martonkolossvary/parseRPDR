###############################################################################
## Title: Test load_dem_old() function
## Project: parseRPDR
## Description: Test load_dem_old() function
...
d_fname <- list.files(paste0(folder_raw, "LEGACY/"), full.names = TRUE)[grep("dem.txt", list.files(paste0(folder_raw, "LEGACY/")), ignore.case = TRUE)]
d_s <- suppressMessages(load_dem_old(d_fname, nThread = 1))
d_p <- suppressMessages(load_dem_old(d_fname, nThread = 2))


test_that("load_dem_old run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "dem_old.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "dem_old.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "dem_old.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
