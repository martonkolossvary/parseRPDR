###############################################################################
## Title: Test load_ptd() function
## Project: parseRPDR
## Description: Test load_ptd() function
...
d_fname <- list.files(folder_raw, full.names = TRUE)[grep("ptd.txt", list.files(folder_raw), ignore.case = TRUE)]
d_s <- suppressMessages(load_ptd(d_fname, nThread = 1))
d_p <- suppressMessages(load_ptd(d_fname, nThread = 2))


test_that("load_ptd run using sequential and parallel loading returns same results", {
...
if(OVERWRITE | !file.exists(paste0(folder_parse, "ptd.csv"))) {data.table::fwrite(d_s, paste0(folder_parse, "ptd.csv"))}
l_s <- data.table::fread(paste0(folder_parse, "ptd.csv"), na.strings = "NA", strip.white = FALSE, colClasses = apply(d_s, 2, class))
